...
**NOTE:** By default, Browserify will only lookup `.js` and `.json` files when the extension is omited (like node's `require`). To lookup additional extensions, use browserify's [`extensions` option](https://github.com/substack/node-browserify#browserifyfiles--opts).
